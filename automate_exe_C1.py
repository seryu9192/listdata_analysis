...
img_folder_names = ['1_COP', '2_charge', '3_C1', '4_Combseparated']
tof_folder_names = ['1_decoded', '2_offsetScale', '3_stop', '4_coinc']
coinc_folder_names = []

img_exe_path = ['./Imaging_1_COPToCharge.exe', './Imaging_2_chargeToCn.exe', './Imaging_3_CnToCombseparated.exe']
        ...
        for i in range(len(tof_exe_path)):
            ...
            if start != end:
                ...
                if i < 2:
                    ...
                    #MCS6A_3_stopToCoinc.exeではイメージングフォルダ名を引数から読むので、引数に'3_C1'を指定
                    code = subprocess.run([exe_path, inputfolder, str(start), str(end-1), '3_C1'])
