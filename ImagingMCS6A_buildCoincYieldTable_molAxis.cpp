//ImagingMCS6A_buildCoincYieldTable_molAxis.cpp: あるTOFのピークとコインシデンスする分子軸の角度の収量を数え、coincYield(chn表記)
//                                          テーブルを作成する

//アップデートログ
//2020.08.09:ImagingMCS6A_countMolAxiswithTOF.cppをもとに作成
//2020.08.24:Regexとmatchしないファイル名を見つけたときの処理をreturn -1 から　continueにした
...
//OUTPUT_DATANAME
string OUTPUT_DATANAME = "_coincYieldTable_molAxis.txt";
...
{
	cout << "*********************************************************************************" << endl;
	cout << "****              ImagingMCS6A_buildCoincYieldTable_molAxis.cpp              ****" << endl;
	...
	for (auto inputfilename_img : fNames)
	{
		...
		// 切り出す角度ごとにカウント
		for (int thetaNum = 0; thetaNum < thetaMin.size(); thetaNum++)
		{
			//角度の範囲を決定
			int tmin = thetaMin[thetaNum];
			int tmax = thetaMax[thetaNum];
			...
			for (int row = 0; row < inputListData_img.size(); row++)
			{
				...
				double molAxis = inputListData_img[row][COL_MOLAXIS];
			...
			//projectile
			vector<int> outputLine = {clusterSize, E0, qcomb, tmin, tmax, 0, incidentCnt};
			...
			for (int siNum = 0; siNum < eventCnt.size(); siNum++)
			{
				...
				vector<int> outputLine = {clusterSize, E0, qcomb, tmin, tmax, m_zList[siNum] ,eventCnt[siNum]};
				outputTableData.push_back(outputLine);
			}
