# automate_exe_C4.py: 一連のデータ変換を自動で行うスクリプト for 4量体(Imaging, TOF)
    ...
    while True:
        ...
        for i in range(len(img_exe_path)):
            ...
            if start != end:
                ...
                #コマンドライン引数に、(inputfolder, start, end)を渡す
        ...
        for i in range(len(tof_exe_path)):
            ...
            if start != end:
                ...
                if i < 2:
                    ...
                    #MCS6A_3_stopToCoinc.exeではイメージングフォルダ名を引数から読むので、引数に'3_C4'を指定
                    code = subprocess.run([exe_path, inputfolder, str(start), str(end-1), '3_C4']) 
