# automate_exe_C2.py: 一連のデータ変換を自動で行うスクリプト(Imaging, TOF)
...
img_folder_names = ['1_COP', '2_charge', '3_C2', '4_Combseparated', '5_C2divangle', '6_molAxis']
tof_folder_names = ['1_decoded', '2_offsetScale', '3_stop', '4_coinc']
coinc_folder_names = []

img_exe_path = ['./Imaging_1_COPToCharge.exe', './Imaging_2_chargeToCn.exe', './Imaging_3_CnToCombseparated.exe', './Imaging_4_C2+CombsepTodivangle.exe', './Imaging_5_C2+divAngleToMolAxis.exe']
...
def main():
    while True:
        ...
        for i in range(len(img_exe_path)):
            inputfolder = os.path.join(working_directory, FOLDER_IMG, img_folder_names[i])
            outputfolder = os.path.join(working_directory, FOLDER_IMG, img_folder_names[i+1])
        ...
        for i in range(len(tof_exe_path)):
            ...
            if start != end:
                ...
                if i < 2:
                    ...
                    #MCS6A_3_stopToCoinc.exeではイメージングフォルダ名を引数から読むので、引数に'3_C2'を指定
                    code = subprocess.run([exe_path, inputfolder, str(start), str(end-1), '3_C2'])
