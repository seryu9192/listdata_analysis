//Imaging_5_C4+SkewerNotationToCircular.cpp: SkewerNotationをもとに、C4+の環状分子を抽出する
//

//アップデートログ
//2020.9.22:Imaging_5_C4+SkewerNotationToCircular.cppから作成

#include "./library.hpp"

//分けるd/lの範囲
//d2/l, d3/lのうち小さいほうがこれより大きい場合、環状と判定
double d_l = 0.4;
...
//outputfolderName
const string OUTPUT_FOLDER_NAME = "\\5-2_Circular";

int main(int argc, char* argv[])
{
	cout << "*****************************************************************" << endl;
	cout << "****        Imaging_5_C4+SkewerNotationToCircular.cpp        ****" << endl;
	...
	for (auto inputFilename : fNames)
	{
		...
		{
			//d2/l or d3/lの値で抽出(小さいほうを取る)
			auto d = min(inputListData[row][COL_d2], inputListData[row][COL_d3]);
			if (d_l <= d)
			{
